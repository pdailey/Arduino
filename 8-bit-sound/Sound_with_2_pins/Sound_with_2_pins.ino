...
void loop() {
  if (millis() > timestamp) { // Is it time yet?
    ...
    int pot = analogRead(HzPin);            // Read the potentiometer connected to analog pin A0 to control alternating flashing speed.
    int freq = map(pot, 0, 1023, minHz, maxHz); // Convert pot analog values to a range from min to max Hz.
